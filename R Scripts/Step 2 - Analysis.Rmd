---
title: "Step 2 - Analysis (figures and tests) - Normalization"
...
#source.dir <- 'C:/Users/Tzviel/Google Drive/eLife - Final Submission/Review/Infants-Analysis-Normalized'
#source.dir <- 'G:/Tzviel Documents/eLife - Final Submission/Review/Infants-Analysis-Normalized'
...
```{r Figure 2 - heatmap TD,fig.height= 12, fig.width=16}
...
A=textGrob("b NTD", gp=gpar(fontface="bold", fontsize = 24))
B=textGrob("a TD", gp=gpar(fontface="bold", fontsize = 24))
# 
...
```{r Figure 4 - Interaction plots (only mother),fig.height = 15, fig.width=10, dpi = 500}
...
```{r Figure 6 - distance mother NTD }
...
### Mors plots of touching (Figure 7) 

Mors plot of the frame in which the infant is touching the mother. 

```{r Figure 7 ,warning = FALSE, echo = FALSE, include = FALSE, fig.height= 17, fig.width=10}
touch.list <- list()
...
### Plotting 
## TD 
TD.touch.plot <- touch.df.TD %>% ggplot(aes(y = 1, x = round(Frame_Touch / (25 * 60) , 2))) + 
  geom_point(size = 2, color = 'blue') + 
    geom_vline(aes(xintercept = round(n / (25 * 60) , 2)), size = 1.5) + 
...
NTD.touch.plot <- touch.df.NTD %>% ggplot(aes(y = 1, x = round(Frame_Touch / (25 * 60) , 2))) + 
...
````{r Figure 8,  fig.height= 17, fig.width=10} 
line.plot <- ggplot(NULL) + geom_abline(slope = 0, intercept = 0.5, size = 3) + opts
null.plot <- ggplot(NULL) + opts
...
box.df <- test.df %>% 
  select(-Name) %>%
 plyr::rename(c('Excursions.100.CM.Per.Minute' = paste('Sqrt (# of excursions \n to mother per minute)  \n (distance to mother smaller \n than 100cm)  \n P =', round(p.vec[1], 4)),
                 'Time.Mother.Radius.100.CM' = paste('Logit proportion of time \n near mother \n (distance to mother smaller \n than 100cm) \n P =', round(p.vec[2], 4)), 
                 'Speed.Prog' = paste('Average progression \n speed (cm/sec) \n P =', round(p.vec[3], 4)), 
                 'Avg.Cover' = paste('Logit proportion \n of room covered \n P =', round(p.vec[4], 4)), 
                 'Touches.Num.Per.Min' = paste('Sqrt (# contact \n episodes per minute) \n  P =', round(p.vec[6], 4)), 
                 'Touches.Duration' = paste('Logit proportion \n of contact time \n  P =', round(p.vec[7], 4)),
                 'Speed.Outside.Mother.Radius.100.CM' = paste('Average speed (cm/sec) outside \n of mother\'s vicinity (distance \n to mother larger than 100cm) \n P =', round(p.vec[5], 4)))) %>% 
  melt(id.var = 'TD') 
...
kable(final.test.data, row.names = FALSE)
