...
for (j in 1:nrow(parameter.DF)) { 
  ...
  count.heat[[j]] <- heatmap.temp$Heatmap
  ...
  labs(caption = paste0(parameter.DF$Name[j])) + 
    theme(plot.caption = element_text(hjust=0.5, size=23))
...
grid.arrange(plot.heat.list[[3]] + opts, plot.heat.list[[5]] + opts, plot.heat.list[[6]] + opts,
             plot.heat.list[[8]] + opts,  plot.heat.list[[11]] + opts
             ,nrow = 2 , ncol = 3)
```

#### Plotting (Figure 3)

```{r Figure 2 b - Heatmap NTD, fig.height= 18, fig.width=16}
...
grid.arrange(plot.heat.list[[1]] + opts, plot.heat.list[[2]] + opts, plot.heat.list[[4]] + opts,
             ...
             plot.heat.list[[9]] + opts, nrow = 3 , ncol = 3 )
```


### Explaination of interaction plot (Figure S1)  
...
### Interaction plots (only mother) (Figure 4 a, b)
...
```{r Figure 3 - Interaction plots (only mother),fig.height = 15, fig.width=10, dpi = 500}
...
### Interaction plots (Figure S3 a, b) 
...
```{r Figure 4 - Interaction plots (full),fig.height = 15, fig.width=10}
...
### Distance From Mother (Figure 5 and 6) 
...
#### Distance from mother - TD (Figure 5)
...
#### Distance from mother - NTD (Figure 6)
...
### Difference in curves (Figure S2 a, b, c, d)
...
## With CI 
CI.cover.plot <- ggplot(df.cover, aes(x = Radius, y = Percentage_Covered, color = Group)) + geom_line(size = 1.5) + geom_ribbon(aes(ymin = Percentage_Covered - SD, ymax = Percentage_Covered + SD, alpha = 0.05 , fill = Group), show.legend = FALSE) +  ggtitle("Average Percent Of Time Spent Near Mother") + 
...
## With CI
CI.vis.plot <- ggplot(df.visit, aes(x = Radius, y = Visits, color = Group)) + geom_line(size = 1.5) + geom_ribbon(aes(ymin = Visits - Full.SD, ymax = Visits + Full.SD, alpha = 0.05 , fill = Group), show.legend = FALSE) +  ggtitle("Average Number Of Visits Per Minute To Mother") +
  labs( y = "Number Of Excursions Per Minute", x ="Distance from mother (cm)")  +  theme(text = element_text(size=22), legend.position = 'none') + opts.perm
...
### Mors plots of touching (Figure 7 a, b) 

Mors plot of the frame in which the infant is touching the mother. 

```{r Figure 6 ,warning = FALSE, echo = FALSE, include = FALSE, fig.height= 17, fig.width=10}
touch.list <- list()
...
````{r Figure 6,  fig.height= 17, fig.width=10} 
line.plot <- ggplot(NULL) + geom_abline(slope = 0, intercept = 0.5, size = 3) + opts
null.plot <- ggplot(NULL) + opts
...
### Comparing different statistics (Figure 8)
...
test.df <- data.frame('Name' = parameter.DF$Name, 
                      ...
                      'Avg.Cover' = Logit(cover.vec), ## logit  
...
org.test.df <- data.frame('Name' = parameter.DF$Name, 
                          ...
                          'Time.Mother.Radius.100.CM' = (time.spent.100), ## logit 
                          'Speed.Prog' = speed.vec.only.prog,
...
final.test.data <- data.frame('Tests' = colnames(test.df[ ,2:8]),
                              'Un-Adjusted P-value' = p.vec, 
                              'BH P-value' = p.vec.adj,
                              'Abs Difference' = diff.vec,
                              'Statistic' = W.vec,
                              'SD' = (sd.vec),
                              'Effect Size' = diff.vec / (sd.vec)) %>% 
  arrange(Un.Adjusted.P.value) 

kable(final.test.data, row.names = FALSE, digits = 4)
...
```{r}
ages.df <- read.csv(file.path(source.dir, 'Input/Age_table.csv'))
colnames(ages.df) <- c('Name', 'Age') 

age.adj <- left_join(test.df, ages.df) 

age.result <- list()
for (i in 2:8) { 
  temp.summary        <- summary(lm(age.adj[ ,i] ~ age.adj$TD + age.adj$Age))
  age.result[[i - 1]]   <- as.data.frame(temp.summary$coefficients[-1, ]) %>% mutate('X' = c('TD', 'AGE'), 'Var' = colnames(age.adj)[i]) %>% select(6, 5, 1:4)

}

bind_rows(age.result) %>% kable(row.names = FALSE, digits = 4)
...
Comparing end-points with t-tests. 
...
test.df <- data.frame('Name' = parameter.DF$Name, 
                      ...
                      'Speed.Outside.Mother.Radius.100.CM' = dist.temp,
                      'Touches.Num' = sqrt(touch.num.temp / parameter.DF$Length), ## square root 
                      'Touches.Duration' = Logit(touch.avg.temp), ## logit 
                      'TD' = parameter.DF[ ,'IS.TD'])
...
Comparing lengths of TD and NTDs session. Not relevant since all endpoints are normalized per time. 

```{r}

t.test(parameter.DF$Length[parameter.DF$IS.TD == TRUE],
       parameter.DF$Length[parameter.DF$IS.TD == FALSE], exact = TRUE)
